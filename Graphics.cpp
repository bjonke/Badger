...
bool CGraphics::Init(HWND chwnd)
{
	...
	if( Player.hIcon == 0 )
	{
		MessageBox(main_window_handle,TEXT("Error, no player.bmp in directory\n"), 
	...
	if( Tile.hIntro == 0 )
	{
		MessageBox(main_window_handle,TEXT("Error, no Acorn.bmp in directory\n"), TEXT("Error"),MB_ICONERROR | MB_OK);
		return false;
	}
	////////////////////////////////////////////////////////////////////
	...
	if( Tile.hBadAcorn == 0 )
	{
		MessageBox(main_window_handle,TEXT("Error, no BadAcorns.bmp in directory\n"), TEXT("Error"),MB_ICONERROR | MB_OK);
		return false;
	}
...
void CGraphics::SplashScreen()
{
	if(Splash)
	{
		...
		DrawText(BackBuffer,"Press SPACE to continue",-1,&rect, DT_CENTER);
...
CGraphics::CGraphics(int x, int y) : FieldX(x), FieldY(y)
{
...
CGraphics::~CGraphics()
{
	DeleteObject(Tile.hIntro);
	DeleteObject(Tile.hBadAcorn);
	DeleteObject(Tile.hAcorn);
	delete Logic;
}
